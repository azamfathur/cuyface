...
export async function analyzeAction(prevState, formData) {
    ...
    if(!imageDataUrl){
        return{
            ok: false,
            html: "<p>Tidak ada foto. Tolong foto dulu dah</p>"
        }
    }
    ...
    if(!OPENROUTER_API_KEY) {
        return{
            ok:false,
            html:'<p>Tambah openrouter API keynya bangg</p>'
        }
    }
...
`;
    ...
    const body = { 
        ...
        messages: [
            {
                role:"system",
                content: " anda asistem untuk analisis foto dengan output HTML ringkas dan aman"
            },
            {
                role: "user",
                content:[
    ...
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions",{
        ...
        headers: {
            ...
            "X-Title": "cuyFace", // Optional. Site title for rankings on openrouter.ai.
